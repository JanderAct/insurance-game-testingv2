...
const EXPECTED_RED: Record<string, { code: number; why: string }> = {
  ...
  'clf-label-backtest-check': {
    code: 1,
    why: '⚠ THIS IS A RULING, NOT AN OUTSTANDING DEFECT, AND THE DISTINCTION IS THE POINT OF THE '
      + 'ENTRY. WC and Property KEEP their calendar-basis tables. Nobody should re-derive them on the '
      + 'strength of the reds below: that was measured, costed and declined. '
      + 'WHAT THE GATE MEASURES: accident-year ultimate over the premium charged for that same year — '
      + 'what the funding slider\'s label actually promises. WHAT THE TABLES ARE: WC_DERIVED and '
      + 'PROPERTY_DERIVED are percentiles of netIncurredLoss / poolPremium, a CALENDAR year, which '
      + 'blends every open accident year at a different maturity and halves the spread. So the two '
      + 'lines are measured against a bar their curves were never built to meet, deliberately. '
      + 'MEASURED at 160 games x 22 years x 4 appetite arms, worst error per band on the gated '
      + 'accident-year basis: WC -7.0 / -11.0 / -7.5, Property +7.9 (thin, ungated) / -4.4 / -1.7, '
      + 'GL +5.2 / -7.6 / -11.6. On the calendar basis the same run reads WC -7.2 / -2.2 / -1.2 and '
      + 'Property +7.5 (thin) / +3.6 / +1.2. '
      + 'WHY WC WAS DECLINED: re-deriving raises every off-Expected stop 3.2% to 6.0%, and only 16.4% '
      + 'of WC accident years written in a ten-year game reach their own runoff horizon before it '
      + 'ends (GL 45.3%, Property 71.0%) — so a WC curve on settled ultimate charges for cost the '
      + 'player never sees land in their own P&L in five games out of six. '
      + 'WHY PROPERTY WAS DECLINED: its indicated accident-year curve moves only -2.6% to +4.0%, '
      + 'inside a point at the stops that carry weight. Re-deriving buys almost nothing. '
      + 'GL IS THE LINE THAT CARRIED THE BASIS CHANGE and it is not part of the ruling: GL_SUPPLIED '
      + 'reads -0.5 to +1.7pp from the 70% stop up against -21.7pp on the calendar basis, and its '
      + 'accident-year ratio CV is 0.4168 against the curve\'s implied 0.3979. Its own residual is '
      + '-11.6pp at the 30% stop, confined to the deliberate-underfunding end where the supplied '
      + 'curve carries a fatter low tail than this model produces. '
      + '⚠ WHAT WOULD RETIRE THIS ENTRY, since a ruling-shaped red cannot retire itself the way the '
      + 'others here do: either re-deriving WC and Property onto the accident-year basis (reversing '
      + 'the ruling, with the premium consequence above), or narrowing what the gate asserts per '
      + 'line. The second is recorded as an open question at the gate rather than taken here — see '
      + 'the note in clf-label-backtest-check.ts on gating each line on the basis its own table was '
      + 'built on, and on what that would stop asserting.',
  },
};
