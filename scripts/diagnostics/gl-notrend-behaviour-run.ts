...
//
//   npx tsx scripts/diagnostics/gl-notrend-behaviour-run.ts   (AT 72ecaa0 ONLY)
//
// WORKTREE-ONLY. Not a shipping diagnostic, not for claims-distribution.
//
// ⚠ THIS FILE DOES NOT RUN AT 4f695a0. It is the PAIRED NO-TREND ARM, and it
// must be run from a worktree detached at 72ecaa0, where GL has no severity
// trend and WAGE_INFLATION_APPLIES.GL is false. It is kept on this branch only
// so the two arms of the comparison sit together. To reproduce:
//   git worktree add ../ripple-glnotrend --detach 72ecaa0
//   cp scripts/diagnostics/gl-notrend-behaviour-run.ts ../ripple-glnotrend/scripts/diagnostics/
//   cd ../ripple-glnotrend && npx tsx scripts/diagnostics/gl-notrend-behaviour-run.ts
//
...
// NO-TREND PAIR, run at 72ecaa0 where GL has no severity trend and
// WAGE_INFLATION_APPLIES.GL is false. Identical seeds, identical decisions.
const GL_SEVERITY_TREND_PER_YEAR = 0;
...
console.log(`GL NO-TREND PAIR (72ecaa0) — ${GAMES} games x ${YEARS} years, GL ONLY, NO reinsurance, 60% funding stop`);
...
console.log(`  SEEDS MATCH gl-trend-behaviour-run.ts EXACTLY — this is the PAIRED no-trend arm.`);
console.log('='.repeat(78));
