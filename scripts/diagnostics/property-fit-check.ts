...
console.log(`  + cat load 0.0247 (ASSERTED)  =  ${(per100 + 0.0247).toFixed(4)} per $100`);
console.log(`  brief states 0.1209 — ${Math.abs(per100 + 0.0247 - 0.1209) < 0.0005 ? 'RECONCILES' : 'DOES NOT RECONCILE'}`);
console.log(`\n  AAL AS AN OUTPUT (not a target):`);
console.log(`    full market   $${(fullTiv * perMillion / 1e6).toFixed(2)}M non-cat  + $${(fullTiv * 0.0247 * 10_000 / 1e6).toFixed(2)}M cat load`);
console.log(`    enrolled med  $${(medEnrolled * perMillion / 1e6).toFixed(2)}M non-cat  + $${(medEnrolled * 0.0247 * 10_000 / 1e6).toFixed(2)}M cat load`);
console.log(`    -> total at the enrolled median $${(medEnrolled * (perMillion + 0.0247 * 10_000) / 1e6).toFixed(2)}M`);
