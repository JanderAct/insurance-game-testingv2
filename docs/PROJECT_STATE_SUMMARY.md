...
**New roster — CANONICAL (200 members) is the game's DEFAULT, committed (Pieces 1–5):**
roster_canonical.csv, $1,300M payroll, ingested as a static checked-in module
(src/data/memberCatalog.ts, generated by scripts/tools/generate-member-catalog.ts) — replaces the
old procedural memberCatalog generator entirely. LUMPY (150) and GRANULAR (450) are scenario
VARIANTS of the same generator at different member counts, not drafts or superseded — retained for
instructor-mode concentration-vs-portfolio teaching contrast. All three are $1,300M payroll.
WC class splits and GL relativities are type-constant lookup tables (WC_CLASS_MIX, GL_RELATIVITIES
in defaultAssumptions.ts), not per-member data — verified to reconcile to the roster's WC/GL columns
exactly (worst residual $50, within the $100 tolerance). Property TIV: PROPERTY_TIV_SCALE
recomputed to 16 for canonical (Option B, preserving Property's relative share of the book).
