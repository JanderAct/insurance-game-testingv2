...
## 3. CLAIM GENERATORS — WC and GL complete, Property outstanding

**WC (src/utils/wcClaimEngine.ts)** and **GL (src/utils/glClaimEngine.ts)** are claim-level
generators replacing the aggregate member-Gamma draw for those lines. Property still runs the
legacy aggregate path and is the last line to convert.
...
- **g_pool** (Gamma(25, 1/25)) is drawn once per year in processYear and shared across all lines.
  WC and GL read it; Property still draws commonLossFactor.
...
Verified results: WC held pure premium $3.7269/$100, gross-basis reconciliation 66.88% vs 66.84%
target. GL held pure premium $6.8305/$100, analytic gross-basis 66.36% (realized 62.01%, reported
not gated due to the alpha=1.3 Pareto tail). Cross-line premium: WC $17.38M (21.6%) / GL $32.79M
(40.7%) / Property $30.30M (37.7%) — Property's figure is NOT comparable, being legacy-path.
...
**Roster — CANONICAL (200 members, $1,300M payroll) is the game's default**, ingested as a static
checked-in generated TypeScript module (src/data/memberCatalog.ts, generated by
scripts/tools/generate-member-catalog.ts from source CSV src/data/roster_canonical.csv), replacing
the old procedural memberCatalog generator. Lumpy (150) and granular (450) are scenario VARIANTS of
the same generator at different member counts, retained for instructor-mode
concentration-vs-portfolio contrast — not drafts, not superseded. WC class splits and GL
relativities are type-constant lookup tables (WC_CLASS_MIX, GL_RELATIVITIES in
defaultAssumptions.ts), not per-member data — verified to reconcile to the roster's WC/GL columns
exactly (worst residual $50, within the $100 tolerance). Property TIV: PROPERTY_TIV_SCALE = 16
(Option B, preserving Property's relative share). Marketplace membership is FIXED at 200 — the
player's pool grows/shrinks within it via status transitions, with a 2-year per-line
re-enrollment cooldown tracked in PoolState.membershipHistory.
...
- **KNOWN, before baseline recapture:** WC opening premium now runs ~$21–25M because
  STARTING_RATE_PER_100.min = 5.00 binds against the canonical roster's rescaled payroll, so
  Property currently sits at ~130% of WC rather than the intended ~80%. Fix is to rescale the
  absolute-dollar starting constants (STARTING_RATE_PER_100, STARTING_FINANCIALS.annualPremium,
  reserve draws) BEFORE recapturing baselines — NOT to re-adjust PROPERTY_TIV_SCALE, which scaled
  correctly (4.64× vs payroll's 4.78×); the ratio flip is a WC under-scaling artifact, not a
  Property error.
